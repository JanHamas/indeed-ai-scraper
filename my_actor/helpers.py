...
def has_next_page(html: str) -> bool:
    """
    True if Indeed's pagination indicates more pages exist beyond the
    current one. Indeed never renders a 'pagination-page-next' testid —
    only 'pagination-page-prev', numbered 'pagination-page-N' links, and
    'pagination-page-current'. So this compares the current page number
    to the highest numbered page link actually present.
    """
    soup = BeautifulSoup(html, "lxml")
    nav = soup.select_one('nav[aria-label="pagination"]')
    if nav is None:
        # No pagination nav at all. This does NOT mean "last page" —
        # it usually means the fetch didn't return a real SERP (bot
        # check, rate limit, truncated response). Callers must not
        # treat this as authoritative on its own; see worker logic.
        return True

    current_el = nav.select_one('[data-testid="pagination-page-current"]')
    if current_el is None:
        return True
    try:
        current_page = int(current_el.get_text(strip=True))
    except ValueError:
        return True

    highest_page = current_page
    for a in nav.select('a[data-testid^="pagination-page-"]'):
        testid = a.get("data-testid", "")
        if testid in ("pagination-page-prev", "pagination-page-current"):
            continue
        num_text = a.get_text(strip=True)
        if num_text.isdigit():
            highest_page = max(highest_page, int(num_text))

    return highest_page > current_page
...
_INDEED_JUNK_PARAMS = {
    "ts", "pts", "from", "rq", "rsIdx", "newcount", "vjk", "advn", "sjdu", "sxu",
}
...
def extract_job_ids_from_urls(urls: list[str]) -> set[str]:
    ids: set[str] = set()
    for url in urls:
...
async def showstartinginfo(config: ScraperConfig) -> None:
    ...
    Actor.log.info(f"🔗 Skip ignore related: {'ON' if config.skip_ignore_related_jobs else 'OFF'}")
